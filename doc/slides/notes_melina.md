ca. 20 Minuten Redezeit -> an Seminar angelegt sollte ca. 240 Zeilen Notizen sein
Dokumentation für Inhalt anschauen! auch kapiteleinleitungen für Begründungen
...
general Aufbau ca. 5 Minuten -> ca. 60 Zeilen (wenn dann Redezeit von der speculative Execution weg nehmen)
    ...
    but firstly I will give a brief overview over how we implemented our emulator as a whole
     ...
     CPU
        CPU class that initializes, coordinates and controls the other components
...
out-of-order-execution ca. 5 Minuten -> ca. 60 Zeilen
    ...
    as Felix already mentioned, we implement out-of-order execution based on Tomasulos algorithm
        ...
        if a subsequent operation in program order has its operands ready before a previous one, it can be executed while the previous instrction is still waiting
            *** vllt. example of three instructions where the second is a mem access that waits on the first one -> execute the third instruction first
    ...
    by putting the SlotIDs of newly issued intructions in their respective target register whenever we issue a instruction, we make sure that the registers always reflect the expected register state at the point of issuing the current instruction, if the program was executed in order
     ...
     when all operands of an instruction are ready, we can execute it
        ...
        in our emulator, we execute instructions as soon as all their operands are ready
     ...
     what I just descibed ensures that data dependencies between instructions based on the fact that they use the same registers are resolved/ adhered to
     also have to take care of memory hazards, out of scope of this presentation, details in our documentation
     
     also winbt go into detail about our special fence instruction, which basically provides a fixpoint in our ooe
        before and after the fence instruction ooe as described above
        but the fence itself is always executed at exactly the point where it sits in program order
            i.e. all previous instructions in program order are actually executed before the fence and all subsequent instructions are actually executed after the fence
    
    dann an Screenshot-Beispiel ruhig dazu sagen, dass die aus der Ausführung eines Beispielprogramms sind, das auch in der Ausarbeitung besprochen wird und in unserem Gitlab Repo zu finden ist
...
speculative execution ca. 5 Minuten -> ca. 60 Zeilen 
    ...
    need two basic components for our speculative execution
    ...
    dann an Screenshot-Beispiel
        neuer Screenshot mit BPU wäre nice :/
        sowohl einmal vor dem Branch BPU
        dann entsprechend gefüllte IQ
        misprediction -> in  more detail afterwards
        BPU again with different predictions

rollbacks ca. 5 Minuten -> ca. 60 Zeilen

    already mentioned in the part about speculative execution that we sometimes mispredict branches and have toroll back the falsely executed instructions
    ...
    why do we even need fault handling?
        ...
        cannot simply reset the pc but have to make sure that we both execute all instructions that precede the faulting instruction in program order and that we take back the effect on the architectural state of the transient executions
